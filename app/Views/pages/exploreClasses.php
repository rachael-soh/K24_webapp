...
<div class="card-header <?php echo $class->class_status != 2? ' text-white '.$color : 'text-white bg-secondary'; ?>"><?php echo $class->class_name?></div>
  <div class="card-body ">
    ...
    <button class="btn btn-lg float-right <?php echo $class->class_status==2? 'd-none' : ''?>" <?php echo $class->class_status != 0 || in_array($class->class_id, $userClasses)? ' disabled ' : ''; ?> type="submit" name="join" value="<?php echo $class->class_id?>"><i class="fas fa-plus-circle"></i></button>
    <button class="btn btn-lg float-right <?php echo session()->get('role_id') != 1 || $class->class_status == 2? ' d-none ' : ''; ?> "  type="submit" name="remove" value="<?php echo $class->class_id?>"><i class="fas fa-minus-circle"></i></button>
  </div>
</div>
<?}}?>
</form>
