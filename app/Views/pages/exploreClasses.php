...
if (isset($classL)){
    ...
    <div style= "background-color: <?php echo $class->color?>" class="card-header <?php echo $class->class_status != 2? ' text-white ' : 'text-white bg-secondary'; ?>"><?php echo $class->class_name?></div>
      <div class="card-body">
        ...
        <button style="border-radius: 60%; border-width: 2px;" class="btn btn-outline-success m-2 float-right <?php echo $class->class_status==2? 'd-none' : ''?>" <?php echo $class->class_status != 0 || in_array($class->class_id, $userClasses)? ' disabled ' : ''; ?> type="submit" name="join" value="<?php echo $class->class_id?>">
        <?php if (in_array($class->class_id, $userClasses)){
        ...
        <button style="border-radius: 60%; border-width: 2px;" class="btn btn-outline-danger m-2 float-right  <?php echo session()->get('role_id') != 1 || $class->class_status == 2? ' d-none ' : ''; ?> "  type="submit" name="remove" value="<?php echo $class->class_id?>"><i class="fas fa-minus"></i></button>
      </div>
    </div>
    </a>
    <?}}?>
