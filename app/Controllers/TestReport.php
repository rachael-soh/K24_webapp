...
class TestReport extends BaseController
{
    public function getScore(){
        ...
        if ($this->request->getMethod() == "post" && in_array(1, $allowed)){
            ...
            // get all classes
                ...
                session()->setFlashdata('class_id', $this->request->getPost("view"));
                return redirect()->to('/k24/public/TestReport/classReport');
            }
        } else {
            session()->setFlashdata('allowed', 'Not allowed to view all report');
            return redirect()->to('/k24/public/TestReport/getScore');
        }
    ...
    public function classReport(){
        ...
        $scores = $testModel->getClassReports(session()->get("class_id"));
        $data['userL'] = $scores;
    ...
    public function testAction(){
        ...
        if ($this->request->getPost('edit-pretest')){
            // edit pretest
            $test_id = $testModel->getTestID(session()->get('class_id'), "pretest");
            $test_id = $test_id[0]->pretest_id;
        ...
        // edit posttest
            ...
            $test_id = $testModel->getTestID(session()->get('class_id'), "posttest");
            $test_id = $test_id[0]->posttest_id;
            $test = $testModel->getTest($test_id);
            if ($test_id == NULL){
                session()->setFlashdata('error', 'Test does not exist');
            } else if ($test->test_status == 3){
        ...
        // take pretest
            ...
            return redirect()->to('/k24/public/TestReport/takeTest');
        } 
        // take posttest
    ...
    public function questionAction(){
        ...
        if ($this->request->getPost('edit')){
            ...
            return redirect()->to("/k24/public/Questions/editQuestion");
        } 
        // delete question
    ...
    public function takeTest(){
        ...
        // if it's pretest, we can just take it
        ...
        // if its posttest AND hasn't been taken before, chek the date & time
